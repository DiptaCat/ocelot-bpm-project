Ocelot BPM PROJECT
===================
Ocelot Bpmn is based on two web applications, Ocelot MT and Ocelot XE, which work as a single one.

Ocelot MT allows the user to create and edit Bpmn models and export them in a xml file or as a svg. The modeling tool has been implemented using the bpmn.io library. This library is still under development but has been very useful and helped us to achieve a good functionality and results. Ocelot MT provides the user with a palette with several items. One of its main features is that it can be configured according to the user needs or way of working.
