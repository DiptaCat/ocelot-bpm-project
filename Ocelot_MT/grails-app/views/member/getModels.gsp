...
<div id="list-model" class="content scaffold-list" role="main">
	...
	<table class="table table-striped table-bordered">
		...
		<tr>
			...
			<g:sortableColumn property="user" title="${message(code: 'model.name.label', default: 'Member')}"/>
		...
		<g:each in="${memberInstance?.models}" status="i" var="model">
			<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
				...
				<td><g:link action="markFavourite" id="${model.id}"
							params="${[userId: memberInstance?.id, modelId: model.id]}"
							controller="member">Add</g:link></td>
